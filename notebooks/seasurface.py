...
def advection_2d(f, u, v, dx, dy):
    ...
    df_dy = (np.roll(f, -1, axis=0) - np.roll(f, 1, axis=0)) / (2.0 * dy)
...
def advection_3d(f, u, v, w, dx, dy, dz):
    df_dx = (np.roll(f, -1, axis=1) - np.roll(f, 1, axis=1)) / (2.0*dx)
    df_dy = (np.roll(f, -1, axis=0) - np.roll(f, 1, axis=0)) / (2.0*dy)
    df_dz = (np.roll(f, -1, axis=2) - np.roll(f, 1, axis=2)) / (2.0*dz)
...
def step_3d(u, v, w, p, T, nu_eff, alpha_eff, dx, dy, dz, dt, beta_g, U_wind,
    ...
    div_star = (np.roll(u_star, -1, axis=1) - np.roll(u_star, 1, axis=1))/(2*dx) + \
               (np.roll(v_star, -1, axis=0) - np.roll(v_star, 1, axis=0))/(2*dy) + \
               (np.roll(w_star, -1, axis=2) - np.roll(w_star, 1, axis=2))/(2*dz)
    ...
    u[:,:,:] = u_star - dt*(np.roll(p,-1,axis=1)-np.roll(p,1,axis=1))/(2*dx)
    v[:,:,:] = v_star - dt*(np.roll(p,-1,axis=0)-np.roll(p,1,axis=0))/(2*dy)
    w[:,:,:] = w_star - dt*(np.roll(p,-1,axis=2)-np.roll(p,1,axis=2))/(2*dz)
